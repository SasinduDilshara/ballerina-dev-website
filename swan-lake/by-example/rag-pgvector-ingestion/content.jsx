...
export function RagPgvectorIngestion({ codeSnippets }) {
    ...
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <blockquote>
        <p>
          ...
          pgvector extension available. For example, start one with Docker using{" "}
          <code>
            docker run --name pgvector-db -e POSTGRES_PASSWORD=&lt;password&gt;
            -e POSTGRES_DB=vector_db -p 5432:5432 -d pgvector/pgvector:pg17
          </code>
          . Add the database configuration to the <code>Config.toml</code> file
          (e.g., <code>pgPassword = &quot;&lt;password&gt;&quot;</code>). This
          example also uses the default embedding provider implementation. To
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run rag_pgvector_ingestion.bal`}</span>
              <span>{`Ingested 3 documents into the 'leave_policy_vectors' table`}</span>
            </code>
          </pre>
        </Col>
      </Row>
