...
export function McpClient({ codeSnippets }) {
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run mcp_client.bal`}</span>
              <span>{`Available tools:`}</span>
              <span>{`- getCurrentWeather: Get current weather for a city.`}</span>
              <span>{`- getWeatherForecast: Get weather forecast for upcoming days.`}</span>
              <span>{`
`}</span>
              <span>{`Current weather in Colombo: {"location":"Colombo","temperature":22.0,"humidity":34,"pressure":1015,"condition":"Sunny","timestamp":"2026-09-21T12:57:26.176151Z"}`}</span>
            </code>
          </pre>
        </Col>
      </Row>
