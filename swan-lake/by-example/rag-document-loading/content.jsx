...
export function RagDocumentLoading({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        ...
        generation (RAG) ingestion workflow. Ballerina provides the{" "}
        <code>ai:DataLoader</code> abstraction to load documents from various
        sources as <code>ai:Document</code> values, which can then be chunked,
        embedded, and indexed in a knowledge base.
      </p>

      <p>
        The built-in <code>ai:TextDataLoader</code> loads files as{" "}
        <code>ai:TextDocument</code>s and supports the <code>pdf</code>,{" "}
        <code>docx</code>, <code>markdown</code>, <code>html</code>, and{" "}
        <code>pptx</code> file types. Additional data loaders are available for
      ...
      <p>
        This example demonstrates how to load documents of different file types
        using <code>ai:TextDataLoader</code> and inspect the loaded documents
        and their metadata.
      </p>
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run rag_document_loading.bal`}</span>
              <span>{`Type: text`}</span>
              <span>{`File name: employee_handbook.md`}</span>
              <span>{`Content preview: # Employee handbook`}</span>
              <span>{`
`}</span>
              <span>{`## Working hours`}</span>
              <span>{`
`}</span>
              <span>{`Regular working hours are from 9:00 AM to...`}</span>
              <span>{`
`}</span>
              <span>{`Type: text`}</span>
              <span>{`File name: leave_policy.pdf`}</span>
              <span>{`Content preview: # 📝 Company Leave Policy`}</span>
              <span>{`
`}</span>
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="https://github.com/ballerina-platform/ballerina-distribution/tree/master/examples/rag-document-loading/employee_handbook.md">
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="https://github.com/ballerina-platform/ballerina-distribution/tree/master/examples/rag-document-loading/leave_policy.pdf">
