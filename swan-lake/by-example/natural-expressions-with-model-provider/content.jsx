...
export function NaturalExpressionsWithModelProvider({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>Natural expressions with a specific model provider</h1>

      <p>
        Natural expressions provide a language-level abstraction for integrating
        with large language models (LLMs). A natural expression can optionally
        specify the <code>ai:ModelProvider</code> to use as an argument, so you
        can use a specific provider with your own keys rather than the default
        model provider.
      </p>

      <p>
        This example demonstrates how to use a natural expression with a
        specific model provider, initialized with your own API key. The example
        uses Anthropic via the{" "}
        <a href="https://central.ballerina.io/ballerinax/ai.anthropic/latest">
          ballerinax/ai.anthropic
        </a>{" "}
        module, but any <code>ai:ModelProvider</code> implementation (e.g.,{" "}
      ...
      <blockquote>
        <p>
          ...
          <code>anthropicApiKey = &quot;&lt;your-api-key&gt;&quot;</code>).
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run --experimental natural_expressions_with_model_provider.bal`}</span>
              <span>{`Contact by Carl Sagan`}</span>
              <span>{`  A scientifically grounded story about humanity's first radio communication with an alien civilization and the journey to meet them.`}</span>
              <span>{`The Left Hand of Darkness by Ursula K. Le Guin`}</span>
              <span>{`  A profound exploration of first contact and cultural understanding as a human envoy visits an alien world with a unique gender-fluid society.`}</span>
              <span>{`Childhood's End by Arthur C. Clarke`}</span>
              <span>{`  A thought-provoking tale of peaceful alien arrival on Earth and the mysterious transformation they guide humanity through.`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="/learn/by-example/natural-expressions/">
              The Natural expressions example
            </a>
          </span>
        </li>
      </ul>
