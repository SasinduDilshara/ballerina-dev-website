...
export function DirectLlmCallsWithOllama({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>Direct LLM calls with a local model using Ollama</h1>

      <p>
        <a href="https://ollama.com/">Ollama</a> allows you to run open-source
        large language models (LLMs) such as Llama, Mistral, and Gemma locally.
        The{" "}
        <a href="https://central.ballerina.io/ballerinax/ai.ollama/latest">
          ballerinax/ai.ollama
        </a>{" "}
        module provides an <code>ai:ModelProvider</code> implementation for
        locally running Ollama models, so the same APIs used with cloud
        providers work with a local model.
      </p>

      <p>
        This example demonstrates how to make direct LLM calls to a model served
        by a local Ollama server.
      </p>

      <blockquote>
        <p>
          Note: Install Ollama, start the Ollama server, and pull the model
          before running the example (e.g., <code>ollama pull llama3.2</code>).
          To use a different model or a server running on another host or port,
          set the <code>ollamaModel</code> and <code>ollamaServiceUrl</code>{" "}
          configurable variables in the <code>Config.toml</code> file.
        </p>
      </blockquote>
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run direct_llm_calls_with_ollama.bal -- city=Kyoto`}</span>
              <span>{`Country: Japan`}</span>
              <span>{`Known for: Temples and gardens`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="/learn/by-example/direct-llm-calls-with-model-provider/">
              The Direct LLM calls with a specific model provider example
            </a>
          </span>
        </li>
      </ul>
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="https://github.com/ollama/ollama/blob/main/README.md">
